...
router.post("/",
  ...
  async (req, res, next) => {
    ...
    try {
      const { title, author, price, category, description, cover_color, in_stock } = req.body;
      const { rows } = await pool.query(
        `INSERT INTO books (title, author, price, category, description, cover_color, in_stock)
         VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
        [title, author, price, category ?? null, description ?? null, cover_color ?? "from-emerald-500 to-teal-600", in_stock ?? true]
      );
...
router.patch("/:id", requireAuth, async (req, res, next) => {
  try {
    const fields = ["title", "author", "price", "category", "description", "cover_color", "in_stock"];
